...
https://www.red-gate.com/simple-talk/blogs/how-to-identify-contention-in-tempdb/
...
https://blog.pythian.com/t-sqlgenerate-statements-to-add-tempdb-datafiles/
...
Select  @X = 1, @BITS = 1
SELECT
@new_tempdbdev_size_MB = 8					-- 8MB, it's easy to increase that after file creation but harder to shrink.
,@new_tempdbdev_Growth_MB = 8				-- 8MB  , can be easily shrunk
...
--  PRINT @NUMPROCS-8
--  print (@NUMPROCS-8)/4 
--print (@NUMPROCS-8)/4 + 8 

 SET @NUMPROCS = CASE 
	WHEN @NUMPROCS >8 THEN 8 -- (@NUMPROCS-8)/4 + 8 
	ELSE @NUMPROCS
	END
 PRINT @NUMPROCS
...
BEGIN
    ...
    SELECT @SQL = 'ALTER DATABASE [tempdb] ADD FILE (NAME = N''tempdev'+CAST (@tempdb_files_count +1 AS VARCHAR (5))+''',FILENAME = N'''+ @new_files_Location + 'temp'+CAST (@tempdb_files_count +1 AS VARCHAR(5)) +'.ndf'',SIZE = '+CAST(@new_tempdbdev_size_MB AS VARCHAR(15)) +'MB,FILEGROWTH = '+CAST(@new_tempdbdev_Growth_MB AS VARCHAR(15)) +'MB )
GO'
    PRINT @SQL
    SET @tempdb_files_count = @tempdb_files_count + 1
END

PRINT 'ALTER DATABASE [tempdb] MODIFY FILE ( NAME = N''tempdev'', SIZE = ' + CAST(@new_tempdbdev_Growth_MB AS VARCHAR(15)) + 'MB, FILEGROWTH = 8MB )'
PRINT 'GO'
PRINT  'ALTER DATABASE [tempdb] MODIFY FILE ( NAME = N''templog'', SIZE = ' + CAST(@new_tempdbdev_Growth_MB AS VARCHAR(15)) + 'MB, FILEGROWTH = 8MB )'