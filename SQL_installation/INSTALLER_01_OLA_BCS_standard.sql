...
LAST UPDATED 12/06/2025 - Gennadi 
OLA Version: 2022-12-03 17:23:44 

Please don't run this script in SQLCMD mode!!!
Otherwise you will get error:

A fatal scripting error occurred.
Variable SRVR is not defined.

BCS Customised Script

Changes:

	USE [master] -> USE [msdb]

	SET @CreateJobs          = 'Y' -> SET @CreateJobs          = 'N' 
	SET @CleanupTime         = NULL -> SET @CleanupTime         = 60 

Bug Fix for output file directory:  

-- AND @Version < 12

File path for backups Changes
	REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE([name],'\',''),'/',''),':',''),'*',''),'?',''),'"',''),'<',''),'>',''),'|',''),' ',' ') AS DatabaseNameFS, -> REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE([name],'\',''),'/',''),':',''),'*',''),'?',''),'"',''),'<',''),'>',''),'|',''),' ','_') AS DatabaseNameFS,
+ Additional Commented out paths

Job definitions customised

--  PATH MOD --------------------------------

      INSERT INTO @CurrentDirectories (ID, DirectoryPath, Mirror, DirectoryNumber, CreateCompleted, CleanupCompleted)
      SELECT ROW_NUMBER() OVER (ORDER BY ID), DirectoryPath 
-- BCS		
	+ 	CASE
			  WHEN @BackupSoftware IS NULL AND @CurrentBackupType = 'FULL' THEN '\FULL'
			  WHEN @BackupSoftware IS NULL AND @CurrentBackupType = 'DIFF' THEN '\DIFF'
			  WHEN @BackupSoftware IS NULL AND @CurrentBackupType = 'LOG' THEN '\TRAN'
			  ELSE ''
		END	  
	  
	  + CASE WHEN RIGHT(DirectoryPath,1) = '\' THEN '' ELSE '\' END
	   --+ CASE WHEN @CurrentAvailabilityGroup IS NOT NULL 
	   --THEN @Cluster + '$' + @CurrentAvailabilityGroup 
	   --ELSE REPLACE(CAST(SERVERPROPERTY('servername') AS nvarchar),'\','$') 
	   --END + '\' 
	   + @CurrentDatabaseNameFS 
	   --+ '\' + UPPER(@CurrentBackupType) 
	   --+ CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END 
	   --+ CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END
	   , Mirror, ROW_NUMBER() OVER (PARTITION BY Mirror ORDER BY ID ASC), 0, 0
      FROM @Directories
      ORDER BY ID ASC
...
SQL Server Maintenance Solution - SQL Server 2005 - 2019
...
IF OBJECT_ID('[dbo].[QueueDatabase]') IS NOT NULL  DROP TABLE [dbo].[QueueDatabase]
...
IF OBJECT_ID('[dbo].[Queue]') IS NOT NULL  DROP TABLE [dbo].[Queue]
...
SET @BackupDirectory     = @dirBACKUP	-- Specify the backup root directory.


SET @BackupDirectory   = NULL

SET @CleanupTime         = 60         -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
...
END
-- end [dbo].[CommandExecute]
GO

-- start [dbo].[DatabaseBackup]
...
END
-- end [dbo].[DatabaseBackup]
GO

-- start [dbo].[DatabaseIntegrityCheck]
...
END
-- end [dbo].[DatabaseIntegrityCheck]
GO

-- start [dbo].[IndexOptimize]
...
BEGIN
  ...
  DECLARE @JobsPrefix nvarchar(max)
  SET @JobsPrefix = 'BCS '
  ...
  SET @JobDescription = @JobsPrefix + 'Database Maintenance Jobs' -- -> SET @JobDescription = 'Source: https://ola.hallengren.com'
  SET @JobCategory = 'Database Maintenance'
  SET @JobOwner = SUSER_SNAME(0x01)

   SET @JobName01 = @JobsPrefix + 'All DBs Full Database Backup (8pm)'
   SET @JobName06 = @JobsPrefix + 'Users DBs DIFF Database Backup'
   SET @JobName04 = @JobsPrefix + 'Users DBs Transaction Log Backup'
   SET @JobName03 = @JobsPrefix + 'All DBs Integrity (1am)'
   SET @JobName02 = @JobsPrefix + 'SQL Server Errorlog Cycle (12am)'
   SET @JobName05 = @JobsPrefix + 'All DBs Optimisation (4m Saturday)'  
  
 /*
  --This delete current jobs:
  ...
  */
  ...
  SET @JobCommand01 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''ALL_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CheckSum = ''Y''' +  CASE WHEN @Version >= 10.5 AND @Compress = 'Y'  THEN ', @Compress = ''Y''' ELSE '' END + ', @CleanupTime = ''96''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
...
DECLARE @schedulename sysname
 IF NOT EXISTS (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName01 + '%')
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName01, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand01, @output_file_name = @OutputFile01
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName01, @name = 'Execute', @enabled = 1, @freq_type = 4, @freq_interval = 1, @freq_subday_type = 1, @freq_subday_interval = 0, @freq_relative_interval = 0, @freq_recurrence_factor = 0, @active_start_time=200000, @active_end_time=235959      
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName01
	EXEC msdb.dbo.sp_update_job @job_name = @JobName01, @enabled = 0
  END
ELSE
  BEGIN
	SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName01 + '%')
	EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName01, @description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner
	EXECUTE msdb.dbo.sp_update_jobstep @step_id= 1, @job_name = @JobName01, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand01, @output_file_name = @OutputFile01
	SET @schedulename= (SELECT c.name FROM msdb.dbo.sysjobs a INNER JOIN msdb.dbo.sysjobschedules b ON a.job_id= b.job_id INNER JOIN msdb.dbo.sysschedules c ON b.schedule_id= c.schedule_id WHERE a.name like @JobName01 + '%')
	EXECUTE msdb.dbo.sp_update_jobschedule @job_name= @JobName01, @name= @schedulename, @new_name = 'Execute'
  END

  SET @JobCommand06 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''DIFF'', @Verify = ''Y'', @CheckSum = ''Y''' +  CASE WHEN @Version >= 10.5 AND @Compress = 'Y' THEN ', @Compress = ''Y''' ELSE '' END + ', @CleanupTime = ''96''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile06 = @OutputFileDirectory + '\' + 'DatabaseBackup_' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
 ...
 IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] like @JobName06 + '%')
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName06, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand06, @output_file_name = @OutputFile06
    --EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName06, @name = 'Execute', @enabled = 1, @freq_type = 4, @freq_interval = 1, @freq_subday_type = 1, @freq_subday_interval = 0, @freq_relative_interval = 0, @freq_recurrence_factor = 0, @active_start_time=200000, @active_end_time=235959      
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName06
	EXEC msdb.dbo.sp_update_job @job_name = @JobName06, @enabled = 0
  END
ELSE 
  BEGIN
	SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName06 + '%')
	EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName06, @description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner
	EXECUTE msdb.dbo.sp_update_jobstep @step_id= 1, @job_name = @JobName06, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand06, @output_file_name = @OutputFile06
  END

  SET @JobCommand04 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''LOG'', @Verify = ''Y'', @CheckSum = ''Y''' +  CASE WHEN @Version >= 10.5 AND @Compress = 'Y' THEN ', @Compress = ''Y''' ELSE '' END + ', @CleanupTime = ''48''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] like @JobName04 + '%')
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName04, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand04, @output_file_name = @OutputFile04
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName04, @name = 'Execute', @enabled = 1, @freq_type = 4, @freq_interval = 1, @freq_subday_type = 8, @freq_subday_interval = 1, @freq_relative_interval = 0, @freq_recurrence_factor = 0     
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName04
	EXEC msdb.dbo.sp_update_job @job_name = @JobName04, @enabled = 0
  END
ELSE
BEGIN
	SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName04 + '%')
	EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName04, @description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner
	EXECUTE msdb.dbo.sp_update_jobstep @step_id= 1, @job_name = @JobName04, @step_name = 'DatabaseBackup', @subsystem = 'CMDEXEC', @command = @JobCommand04, @output_file_name = @OutputFile04
	SET @schedulename= (SELECT c.name FROM msdb.dbo.sysjobs a INNER JOIN msdb.dbo.sysjobschedules b ON a.job_id= b.job_id INNER JOIN msdb.dbo.sysschedules c ON b.schedule_id= c.schedule_id WHERE a.name like @JobName04 + '%')
	EXECUTE msdb.dbo.sp_update_jobschedule @job_name= @JobName04, @name= @schedulename, @new_name = 'Execute'
END

  SET @JobCommand03 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''ALL_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile03 = @OutputFileDirectory + '\' + 'DatabaseIntegrityCheck_' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] like @JobName03 + '%')
  BEGIN
     ...
     EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName03, @step_name = 'DatabaseIntegrityCheck', @subsystem = 'CMDEXEC', @command = @JobCommand03, @output_file_name = @OutputFile03
     EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName03,  @name = 'Execute', @enabled = 1, @freq_type = 4, @freq_interval = 1, @freq_subday_type = 1, @freq_subday_interval = 0, @freq_relative_interval = 0, @freq_recurrence_factor = 0, @active_start_time=010000, @active_end_time=235959      
	 EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName03
  END
  ELSE 
   BEGIN
  SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName03 + '%')
  EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName03,@description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner
  EXECUTE msdb.dbo.sp_update_jobstep @step_id= 1, @job_name = @JobName03,  @step_name = 'DatabaseIntegrityCheck', @subsystem = 'CMDEXEC', @command = @JobCommand03, @output_file_name = @OutputFile03
  SET @schedulename= (SELECT c.name FROM msdb.dbo.sysjobs a INNER JOIN msdb.dbo.sysjobschedules b ON a.job_id= b.job_id INNER JOIN msdb.dbo.sysschedules c ON b.schedule_id= c.schedule_id WHERE a.name like @JobName03 + '%')
	EXECUTE msdb.dbo.sp_update_jobschedule @job_name= @JobName03, @name= @schedulename, @new_name = 'Execute'
  END

  --SET @JobName05 = 'BCS All DBs Optimisation & Integrity'
  ...
  --SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''ALL_DATABASES'', @LockTimeout = 600, @UpdateStatistics = ''ALL'', @OnlyModifiedStatistics = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''ALL_DATABASES'', @SortInTempdb = ''Y'', @FragmentationMedium = ''INDEX_REORGANIZE'', @FragmentationHigh = ''INDEX_REBUILD_OFFLINE'', @FragmentationLevel1 = 5, @FragmentationLevel2 = 30, @UpdateStatistics = ''ALL'', @OnlyModifiedStatistics = ''Y'', @LockTimeout = 1800' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] like @JobName05 + '%')
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName05, @step_name = 'IndexOptimize', @subsystem = 'CMDEXEC', @command = @JobCommand07, @output_file_name = @OutputFile07 
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName05, @name = 'Execute', @enabled = 1, @freq_type = 8, @freq_interval = 64,  @active_start_time = 40000, @freq_recurrence_factor = 1 --, @freq_subday_type = 1, @freq_subday_interval = 0, @freq_relative_interval = 0     
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName05
  END
  ELSE
   BEGIN
  SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName05 + '%')
  EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName05, @description = @JobDescription, @category_name = @JobCategory, @owner_login_name = @JobOwner
  EXECUTE msdb.dbo.sp_update_jobstep @step_id= 1, @job_name = @JobName05, @step_name = 'IndexOptimize', @subsystem = 'CMDEXEC', @command = @JobCommand07, @output_file_name = @OutputFile07 
  SET @schedulename= (SELECT c.name FROM msdb.dbo.sysjobs a INNER JOIN msdb.dbo.sysjobschedules b ON a.job_id= b.job_id INNER JOIN msdb.dbo.sysschedules c ON b.schedule_id= c.schedule_id WHERE a.name like @JobName05 + '%')
	EXECUTE msdb.dbo.sp_update_jobschedule @job_name= @JobName05, @name= @schedulename, @new_name = 'Execute'
  END
  
  SET @JobCommand08 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + 'msdb' + ' -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) EXECUTE dbo.sp_delete_backuphistory @oldest_date = @CleanupDate" -b'
  ...
  SET @JobCommand09 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + 'msdb' + ' -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) EXECUTE dbo.sp_purge_jobhistory @oldest_date = @CleanupDate" -b'
  ...
  SET @JobCommand10 = 'cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @OutputFileDirectory + '" /m *_*_*_*.txt /d -42 2^>^&1'') do if EXIST "' + @OutputFileDirectory + '"\%v echo del "' + @OutputFileDirectory + '"\%v& del "' + @OutputFileDirectory + '"\%v"'
  ...
  SET @JobCommand11 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "DELETE FROM [dbo].[CommandLog] WHERE StartTime < DATEADD(dd,-42,GETDATE())" -b'
  ...
  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] like @JobName02 + '%')
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName02, @description = @JobDescription, @category_name = '[Uncategorized (Local)]', @owner_login_name = @JobOwner
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'Change job history', @subsystem = 'TSQL', @database_name=N'msdb', @command = N'EXEC msdb.dbo.sp_set_sqlagent_properties @jobhistory_max_rows=-1, @jobhistory_max_rows_per_job=-1', @on_success_action = 3
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'Cycle ERRORLOG', @subsystem = 'TSQL', @command = N'exec master.dbo.sp_cycle_errorlog', @database_name = N'master', @on_success_action = 3, @retry_attempts = 5, @retry_interval = 3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'Clean backup history', @subsystem = 'CMDEXEC', @command = @JobCommand08, @output_file_name = @OutputFile08, @on_success_action = 3
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'Clean job history', @subsystem = 'CMDEXEC', @command = @JobCommand09, @output_file_name = @OutputFile09, @on_success_action = 3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name=  'Clean Database Mail history', @subsystem=N'CMDEXEC', @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d msdb -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) EXECUTE dbo.sysmail_delete_mailitems_sp @sent_before = @CleanupDate  EXECUTE dbo.sysmail_delete_log_sp @logged_before = @CleanupDate" -b', @on_success_action = 3   
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name=  'Clean SSIS Catalog', @subsystem=N'CMDEXEC', @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d msdb -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) DELETE FROM msdb.dbo.sysssislog where endtime < @CleanupDate DELETE FROM msdb.dbo.sysmaintplan_logdetail where end_time < @CleanupDate DELETE FROM msdb.dbo.sysmaintplan_log where end_time < @CleanupDate" -b', @on_success_action = 3   
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'OutputFileCleanup', @subsystem = 'CMDEXEC', @command = @JobCommand10, @output_file_name = @OutputFile10, @on_success_action = 3
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = 'CommandLogCleanup', @subsystem = 'CMDEXEC', @command = @JobCommand11, @output_file_name = @OutputFile11
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName02, @name = 'Execute', @enabled=1, @freq_type=4, @freq_interval=1, @freq_subday_type=1, @freq_subday_interval=0, @freq_relative_interval=0, @freq_recurrence_factor=0, @active_start_date=20100823, @active_end_date=99991231, @active_start_time=0, @active_end_time=235959    
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName02
  END
ELSE
BEGIN
	SET @jobname= (SELECT name FROM msdb.dbo.sysjobs WHERE [name] like @JobName02 + '%')
	EXECUTE msdb.dbo.sp_update_job @job_name = @jobname, @new_name=  @JobName02,@description = @JobDescription, @category_name = '[Uncategorized (Local)]', @owner_login_name = @JobOwner
    EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name = 'Change job history', @step_id = 1, @command = N'EXEC msdb.dbo.sp_set_sqlagent_properties @jobhistory_max_rows=-1, @jobhistory_max_rows_per_job=-1'
    EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name = 'Cycle ERRORLOG', @step_id = 2, @command = N'exec master.dbo.sp_cycle_errorlog'
 	EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name = 'Clean backup history', @step_id = 3, @command = @JobCommand08, @output_file_name = @OutputFile08
    EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name = 'Clean job history', @step_id = 4, @command = @JobCommand09, @output_file_name = @OutputFile09
	EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name=  'Clean Database Mail history', @step_id = 5, @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d msdb -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) EXECUTE dbo.sysmail_delete_mailitems_sp @sent_before = @CleanupDate  EXECUTE dbo.sysmail_delete_log_sp @logged_before = @CleanupDate" -b'   
	EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_name=  'Clean SSIS log', @step_id = 6, @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d msdb -Q "DECLARE @CleanupDate datetime SET @CleanupDate = DATEADD(dd,-42,GETDATE()) DELETE FROM msdb.dbo.sysssislog where endtime < @CleanupDate DELETE FROM msdb.dbo.sysmaintplan_logdetail where end_time < @CleanupDate DELETE FROM msdb.dbo.sysmaintplan_log where end_time < @CleanupDate" -b'
	EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_id = 7, @command = @JobCommand10
    EXECUTE msdb.dbo.sp_update_jobstep @job_name = @JobName02, @step_id = 8, @command = @JobCommand11
	SET @schedulename= (SELECT c.name FROM msdb.dbo.sysjobs a INNER JOIN msdb.dbo.sysjobschedules b ON a.job_id= b.job_id INNER JOIN msdb.dbo.sysschedules c ON b.schedule_id= c.schedule_id WHERE a.name like @JobName02 + '%')
	EXECUTE msdb.dbo.sp_update_jobschedule @job_name= @JobName02, @name= @schedulename, @new_name = 'Execute'
  END

END
--GO

/** CREATE ADHOC / ADDITIONAL JOBS

Alter one of the above jobs definitions and execute.
Then alter the job schedules as appropriate
